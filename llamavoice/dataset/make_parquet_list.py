...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_utts_per_parquet", type=int, default=1000, help="num utts per parquet"
    )
    parser.add_argument(
        "--num_processes", type=int, default=1, help="num processes for make parquets"
    )
    ...
    for i, j in enumerate(range(0, len(utts), args.num_utts_per_parquet)):
        ...
        utt2parquet_file = os.path.join(
            args.des_dir, "utt2parquet_{:09d}.json".format(i)
        )
        spk2parquet_file = os.path.join(
            args.des_dir, "spk2parquet_{:09d}.json".format(i)
        )
    ...
    with open("{}/data.list".format(args.des_dir), "w", encoding="utf8") as f1, open(
        "{}/utt2data.list".format(args.des_dir), "w", encoding="utf8"
    ) as f2, open("{}/spk2data.list".format(args.des_dir), "w", encoding="utf8") as f3:
